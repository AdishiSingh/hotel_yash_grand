...
function generatePrompt(name, description, category) {
  let plating = "served on a matte black ceramic plate";
  if (category === "Soup" || category === "Dal") {
    plating = "served in a matte black ceramic bowl";
  } else if (category === "Beverages") {
    const nameLower = name.toLowerCase();
    if (nameLower.includes("tea") || nameLower.includes("lassi") || nameLower.includes("milk") || nameLower.includes("lime") || nameLower.includes("coffee")) {
      if (nameLower.includes("tea") || nameLower.includes("lassi")) {
        plating = "served in a traditional Indian clay kulhad (terracotta cup)";
      } else {
        plating = "served in a premium textured glass cup";
      }
    } else {
      plating = "served in a premium crystal glassware";
    }
  }

  const baseStyle = "Ultra photorealistic food photography, premium restaurant presentation, warm ambient lighting, soft depth of field, DSLR quality, 8K resolution, natural garnish, no people, no text, no watermark, consistent camera angle.";

  return `${name} - ${description || "Premium Indian culinary dish"}. Beautifully arranged and ${plating} on a dark walnut wooden table. ${baseStyle}`;
}

function runPipeline() {
  console.log("Starting menu image automation pipeline...");

  if (!fs.existsSync(MENU_FILE_PATH)) {
    console.error(`Error: Menu file not found at ${MENU_FILE_PATH}`);
    process.exit(1);
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentItem) {
      ...
      if (imgMatch) {
        currentItem.imageLineIndex = i;
      }

      // Detect single item boundary end
      if (line.trim() === "}," || line.trim() === "}") {
        if (currentItem.id && currentItem.slug && currentItem.category) {
          const categorySlug = CATEGORY_MAP[currentItem.category] || currentItem.category.toLowerCase().replace(/[^a-z0-9]+/g, "-");
          const imagePath = UPLOADS[currentItem.id] || `/assets/food/${categorySlug}/${currentItem.slug}.webp`;
          
          // Update the line in updatedLines array
          if (currentItem.imageLineIndex !== -1) {
            const originalLine = lines[currentItem.imageLineIndex];
            const leadingSpaces = originalLine.match(/^\s*/)[0];
            updatedLines[currentItem.imageLineIndex] = `${leadingSpaces}image: "${imagePath}",`;
          }

          // Generate Prompt
          const prompt = generatePrompt(currentItem.name, currentItem.description, currentItem.category);
  ...
  fs.writeFileSync(MENU_FILE_PATH, updatedLines.join("\n"), "utf8");
  console.log(`Updated image paths for ${processedItems.length} items in menu.ts.`);

  // Write prompts JSON mapping
  fs.writeFileSync(PROMPTS_FILE_PATH, JSON.stringify(processedItems, null, 2), "utf8");
  console.log(`Generated prompts mapping file at ${PROMPTS_FILE_PATH}`);
