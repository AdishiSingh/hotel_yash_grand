...
export function BookingCTA({ onBookClick }: BookingCTAProps) {
  ...
  const handleWhatsApp = () => {
    requireAuth((customer) => {
      const text = encodeURIComponent(`Hello Hotel Yash Grand! I would like to inquire about booking the Banquet Hall for an upcoming event. Guest: ${customer.name}`);
    ...
    <div className="relative w-full rounded-2xl overflow-hidden border border-white/5 py-20 px-8 sm:px-12 text-center md:text-left select-none shadow-lux mt-28">
      ...
      <div className="absolute inset-0 z-0 select-none pointer-events-none">
        <Image
          ...
          alt="Luxury wedding stage decor backdrop"
          fill
          sizes="100vw"
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-12 gap-8 items-center max-w-6xl mx-auto">
        <div className="md:col-span-8 space-y-4">
          ...
          <p className="text-xs sm:text-sm text-neutral-400 font-sans font-light leading-relaxed max-w-xl">
            Meet our event coordinators on-site to inspect the ballroom space, review catering menus, and calculate estimates for your upcoming marriage celebrations.
          </p>
        </div>
